...
export default function Dashboard() {
  ...
  const quick = [
    { title: 'My hardware', desc: 'Assignments tied to your profile.', to: '/my-assets', show: true, icon: Laptop },
    ...
    <div className="p-6 sm:p-8 max-w-6xl mx-auto space-y-10">
        ...
        {stats
          ...
          .map(({ label, value, sub, icon: Icon, onClick }) => (
            <button
              ...
              type="button"
              onClick={onClick}
              className="text-left rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm hover:shadow-lg hover:border-teal-400 hover:-translate-y-1 transition-all duration-200 group cursor-pointer"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="rounded-xl bg-slate-50 p-2.5 text-slate-600 group-hover:bg-teal-50 group-hover:text-teal-700 transition-colors">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {quick.map(({ title, desc, to, icon: Icon }) => (
            <button
              key={to}
              type="button"
              onClick={() => navigate(to)}
              className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-5 text-left hover:border-teal-400 hover:shadow-md hover:-translate-y-1 transition-all duration-200 cursor-pointer group"
            >
              <div className="shrink-0 h-11 w-11 rounded-xl bg-gradient-to-br from-slate-100 to-slate-50 flex items-center justify-center text-slate-700">
                <Icon className="w-5 h-5" />
              </div>
              <div>
